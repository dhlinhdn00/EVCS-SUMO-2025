...
def generateNetwork(
    ...
    """
    Run full pipeline using merged region JSON.

    Args:
        base_folder: Directory to write outputs
        regions_json: Path to merged JSON with {region: {id, color, communes}}
        buffer_degree: Amount to expand the global bounding box
    """
    ...
    log_file      = base / "logs" / f"generate_network.log"

    logger = _setupLogging(log_file)
    logger.info("Pipeline start for %s", file_name)

    # Load combined region data
    ...
    for region, info in regions_data.items():
        ...
        if not communes:  # Skip regions with no communes (e.g., 'outside')
            continue

        s, w, n, e = 90.0, 180.0, -90.0, -180.0
        for commune in communes:
            logger.info("Fetching bbox for %s (%s)", commune, region)
            try:
                ...
                logger.error("Error fetching bbox for %s: %s", commune, ex)
    ...
    #### Fetch highways and generate network
    ...
    if proc.stderr: logger.warning("netconvert stderr:\n%s", proc.stderr)
    proc.check_returncode()
    logger.info("Network generated: %s", net_xml)

    #### Generate polygons
    tree = ET.parse(str(net_xml)); root = tree.getroot()
    ...
    for region, info in regions_data.items():
        ...
        for commune in communes:
            ...
            try:
                r = requests.get(_OVERPASS_API_URL, params={'data': _POLYGON_QUERY_TEMPLATE.format(commune=commune)}, timeout=60)
                r.raise_for_status(); elems = r.json()['elements']
                ...
                for i, poly in enumerate(polygonize(merged)):
...
def splitNetwork(
    ...
    # ---------------------------------------------------------------------
    # Resolve paths and sanity‑check
    # ---------------------------------------------------------------------
    base = Path(base_folder)
    if not base.is_dir():
    ...
    # ---------------------------------------------------------------------
    # 1. Filter <poly> elements whose type is in region_ids
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # 2. Build a single bounding rectangle string for --keep-edges.in-boundary
    # ---------------------------------------------------------------------
    bbox_str = f"{union_minx:.2f},{union_miny:.2f},{union_maxx:.2f},{union_maxy:.2f}"

    # ---------------------------------------------------------------------
    # 3. Run netconvert to trim the network
    # ---------------------------------------------------------------------
...
def makeNetworkConnected(base_folder: str) -> Path:
    ...
    # Step A: Run netcheck on original network to find all components
    logger.info("Step A: netcheck on original network to compute connected components")
    # Ensure components_txt1 exists (empty)
    ...
    # Step B: extractMaxComponent → write only largest component edges into keep_edges_txt1
    ...
    # Step 1 (P2_1): netconvert → keep passenger edges + remove isolated, using keepEdges from Step B
    ...
    # Step 2 (P2_2): netconvert → keep only edges listed in keep_edges_txt1
