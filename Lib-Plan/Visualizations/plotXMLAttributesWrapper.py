...
def getOptions(args=None):
    ...
    optParser.add_option("--cumulative-x", action="store_true", default=False, help="Compute cumulative sum of X values before plotting")
    optParser.add_option("--cumulative-y", action="store_true", default=False, help="Compute cumulative sum of Y values before plotting")
...
def main(options):
    ...
    for dataID, d in data.items():
        ...
        if options.cumulative_x:
            xvalues = list(itertools.accumulave(xvalues))
            d[xdata] = xvalues
        if options.cumulative_y:
            yvalues = list(itertools.accumulate(yvalues))
            d[ydata] = yvalues
